...
Can the IsoScope VQ **latent** re-align a 3D stack whose slices have been
misaligned by random affine transforms (the serial-section scenario: each
section lands on the previous one with a small random rotation/shift/scale)?
Yes — including directly from the stored compressed **codec**.

**Why it works:** the encoder is strictly 2D per-slice (`models/MSclean.py`
`vol_to_slices`), so each XY slice owns an independent `(4, Y/8, X/8)` latent
plane, and affine geometry survives the convolutional encoder (rotation → same
rotation; translation `t` px → `t/8` latent px). So slices align *in latent
space*, with no raw data needed for the codec path.

## Pipeline (env `py38zarr`; `$OUT=/home/cheese/workspace/Output`)

```bash
# 1. corrupt a registered sample0 ROI with a per-slice similarity random-walk
...
**What holds up:**
- **Registration works**: misalignment drops ~3× (translation) / ~4×
  (rotation); adjacent-slice coherence returns to the pristine level. `xcorr`
  fails (can't handle rotation).
- **Codec ≈ latent**: a misaligned stack can be re-registered from the
  compressed codes alone — no raw volume, no re-encode. This is the headline;
  pixels can't do it.
