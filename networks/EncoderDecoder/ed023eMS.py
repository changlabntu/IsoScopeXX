...
# ed023eMS: ed023e with extra coarse-resolution output heads for progressive/multi-scale
# 3D decoding. The decode path (up3, conv5, up2, conv6, up1) has no skip connections, so
# intermediate stages are pure coarse-resolution features that can be tapped by extra heads:
#   after conv5 -> 1/4 res (4*nf ch) -> conv7_64  -> 'out64'
#   after conv6 -> 1/2 res (2*nf ch) -> conv7_128 -> 'out128'
#   after up1   -> full res (nf ch)  -> conv7_k   -> 'out0' (unchanged)
