...
class PatchNCELoss(nn.Module):
    ...
    def forward(self, feat_q, feat_k):
        ...
        diagonal = torch.eye(npatches, dtype=self.mask_dtype, device=feat_q.device)[None, :, :]#.cuda()
...
def define_F(input_nc=3, netF='mlp_sample', norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    if netF == 'global_pool':
        net = PoolingF()
    elif netF == 'reshape':
        net = ReshapeF()
