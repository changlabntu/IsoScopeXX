...
# uncertain foreground boundary — lands in out/std/{tag}/{stem}_maskstd.tif.
# concat_views.py --std then builds the same two-panel view concats as the
# mean-output summary into:
#   out/summarystd/{tag}.tif    [ZX page y=k | XY page z=k] of the maskstd map
# The mean outputs this run also produces overwrite out/{tag}/{stem}.tif with
# the 8-pass average (same convention as inference3d.sh with --tta).
...
for i in "${!TAGS[@]}"; do
  CUDA_VISIBLE_DEVICES=0 NO_ALBUMENTATIONS_UPDATE=1 python test/inference.py \
    ...
    --source "$SRC/$STEM.tif" --destination "$OUT/${TAGS[i]}" \
