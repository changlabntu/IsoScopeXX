...
**Multi-view fused data** (`E2507218fuse`, `--direction zcube_xcube_ycube`) is a regime where the anisotropy is *real*, not faked: three registered views of the same sample, each 8× low-res along a different axis (zcube→Z, xcube→X, ycube→Y). The HR-Z structure missing from zcube is present in xcube/ycube, enabling **real projection supervision** of the Z output. This is implemented in **`models/vqcleanM0aSup0.py`** (run with `--models vqcleanM0aSup0 --direction zcube_xcube_ycube --nm 11p --lamb_xy <w> --aniso 8`): `generation()` captures xcube/ycube at full Z (before the `dsp` subsample), and `backward_g` mean-pools the isotropic output `XupX` along X (dim3)→xcube and along Y (dim2)→ycube by `--aniso`, L1-matching the real views to supervise `XupX`'s Z with genuine HR structure (weighted by `--lamb_xy`; `--nm 11p` puts all views on a shared intensity scale). **`models/MScleanSup0.py`** ports the same supervision onto the current `MSclean` recipe (`--models MScleanSup0`, defaults `--l1how_xy mean` — mean-pooling commutes with the views' box-average forward model, `max` biases toward blurred X/Y; no new parameters, so plain `MSclean` checkpoints load directly; geometry constraint: `cropz == cropsize`, `dsp == aniso == 8`, `--downbranch 1`, since `backward_g` asserts `XupX` and the views share one grid). **`models/MScleanSup1.py`** replaces the box/max projection with the *measured* forward model for fused data (axial PSF ≈ Gaussian σ≈12 vox, FWHM≈28, fitted from the views themselves — the 8 is only the sampling ratio): `--l1how psf --psf_sigma 12` blurs `XupX` along Z and samples the input's own planes for the main `l1`, and the side losses blur along X/Y before comparing with xcube/ycube, with per-sample gain fit (`--side_gain`) and ±2-vox shift tolerance (`--side_shift`). Under the box model a correct sharp output can never match the blurrier data, so every projection loss pushes toward blur.

## Running training

There is no build step and no test suite. The entry point is `train.py`, usually launched via copy-pasting a line from `run.sh` (the **last uncommented line is the current focused experiment** — currently the multi-scale `vqcleanM0aMS` run).

```bash
CUDA_VISIBLE_DEVICES=2 NO_ALBUMENTATIONS_UPDATE=1 python train.py \
  --yaml aisr --models vqcleanM0aMS --prj <output/subdir> --env brcb \
...
So model **hyperparameters** live in `cfg/*.yaml`, but the **autoencoder network shape** lives in `ldm/*.yaml`. MLflow tracking: an `http://` `--tracking_uri` (or `TRACKING_URI` in env.json) must pass a `/health` check or training aborts; otherwise `--tracking_uri` is treated as a bare **NAME** and resolves locally to `$LOGS/NAME.db` + `$LOGS/NAME/` (default name `mlflow`).
...
**`dataloader/data_multi.py` → `PairedImageDataset`** loads 3D TIFFs. `--direction` is split on `_` to address multiple paired sub-directories under `train/`/`val/` (e.g. `zcube_xcube_ycube`); files are paired by **shared filename** across those dirs. Albumentations applies precrop → resize → rotate → random crop. A missing `val/` directory is tolerated (training proceeds without validation).
