...
# vqcleanM0aMSfpn: vqcleanM0aMS + FPN-style scale-to-depth injection linking the residual
# VQ scales to the progressive output heads. Requires --netG ed023emsfpn and num_scales >= 3.
#
# In vqcleanM0aMS all VQ scales are summed into one latent that every output head decodes,
# so the coarse codes are never forced to carry coarse structure. Here the 3D generator's
# trunk input is only the COARSE partial sum, and the two finest scales are injected
# laterally (1x1 Conv3d adapter + trilinear resize inside net_g) at the decoder depths
# AFTER the coarse heads' taps:
#   trunk input <- sum(quants[:num_scales-2])   -> out64  sees coarse scales only
#   quants[-2]  -> injected before up2          -> out128 sees coarse + one finer scale
#   quants[-1]  -> injected before up1          -> out0   alone sees all scales
# The 2D VQGAN reconstruction path still uses the full sum, so aeloss is unchanged.
# The coarse heads are supervised as in vqcleanM0aMS (pyramid consistency --lamb_pyr /
# --pyr_detach, per-scale six-way GANs --adv_ms), but the heads now have genuinely
# restricted code visibility, so those losses train the codes, not just deep features.
class GAN(BaseModel):
    def __init__(self, hparams, train_loader, eval_loader, checkpoints):
        ...
        if self.num_scales < 3:
            raise ValueError('vqcleanM0aMSfpn needs --num_scales >= 3 (coarse trunk + two '
                             'injected scales); got %d' % self.num_scales)
    ...
    def generation(self, batch, deterministic=False):
        ...
        # The net_g trunk sees only the coarse partial sum; the two finest scales are
        # injected laterally after the out64/out128 taps (2D recon above used the full sum).
        quant = sum(quants[:self.num_scales - 2])
