...
# input here is the FULL sum of all VQ scales — every code gets full decoder depth, so the
# quality floor is the vqcleanM0aMS baseline. The two finest scales are ADDITIONALLY
# injected laterally (1x1 Conv3d adapter + trilinear resize inside net_g) after the
# out64/out128 taps, giving high-frequency codes a shortcut past ~8 trunk layers:
#   trunk input <- sum(quants)     (all scales, like vqcleanM0aMS)
#   quants[-2]  -> also injected before up2
#   quants[-1]  -> also injected before up1
# The adapters are ZERO-INITIALIZED (ControlNet-style): at step 0 the skips contribute
# nothing and the model is numerically identical to vqcleanM0aMS, so there is no
# cold-start handicap — training can only learn skips that help. The visibility contract
# of the fpn variant is intentionally given up; the coarse heads remain plain taps.
class GAN(BaseModel):
    def __init__(self, hparams, train_loader, eval_loader, checkpoints):
        ...
        if self.num_scales < 3:
            raise ValueError('vqcleanM0aMSskip needs --num_scales >= 3 (two of the scales '
