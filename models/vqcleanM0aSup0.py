# vqcleanM0aSup0 — supervised variant of vqcleanM0a for multi-view fused data.
#
# Same model as vqcleanM0a, plus projection supervision from the extra views:
# with --direction zcube_xcube_ycube the loader yields data0=zcube (input, LR in Z),
# data1=xcube (LR in X / HR in Z), data2=ycube (LR in Y / HR in Z). The isotropic
# output XupX is mean-projected along X (tensor dim3) to match xcube and along Y
# (dim2) to match ycube; because those pools keep Z full-res and the cubes are HR in
# Z, this supervises XupX's Z with real high-res structure. Weighted by --lamb_xy.
# Requires --nm 11p so the three views share an intensity scale (see docs/fuse_data.md).
...
class GAN(BaseModel):
    ...
    def add_model_specific_args(parent_parser):
        ...
        # Multi-view projection supervision (xcube/ycube)
    ...
    def get_projection(self, x, depth, how='mean', axis=-1):
        if how == 'dsp':
