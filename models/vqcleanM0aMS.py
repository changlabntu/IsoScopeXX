...
# vqcleanM0aMS: vqcleanM0a + multi-scale (progressive) 3D output heads.
#
# The 3D generator (--netG ed023ems) emits the isotropic volume at three resolutions from
# different decoder depths: full (out0), 1/2 (out128), and 1/4 (out64). The coarse heads are
# supervised by:
#   - pyramid consistency: L1 to an avg-pooled version of the full-res output (--lamb_pyr;
#     gradient flows both ways by default so the coarse heads also shape the main output,
#     unless --pyr_detach makes it distillation-only), and
#   - a per-scale six-way adversarial loss from dedicated discriminators (net_d_128 / net_d_64),
#     each scored against real X/Y slices downsampled to the matching resolution.
class GAN(BaseModel):
